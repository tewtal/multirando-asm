...
def main_dma_allocation(data):
    lorom = [(bank*0x10000+0x8000,bank*0x10000+0x10000) for bank in range(0x9C,0xA0)]
    hirom = [(bank*0x10000,bank*0x10000+0x10000) for bank in range(0xD1,0xD6)]
    merged = lorom + hirom

    freespace = FreeSpace(merged)
...
class FreeSpace():
    def __init__(self, blocks):
        ...
        info = [f"{left:x}, {right:x}" for (left,right) in blocks]

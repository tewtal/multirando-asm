...
cm_init:
{
    ...
    %a16()
    LDA #$0000
    STA.l !ram_cm_stack_index : STA !ram_cm_cursor_stack
    STA !ram_cm_leave
    STA !ram_cm_ctrl_mode : STA !ram_cm_ctrl_timer
    STA.l !IH_CONTROLLER_PRI_NEW : STA.l !IH_CONTROLLER_PRI
...
cm_transfer_custom_tileset:
{
    ...
    ; Enable force-blank
    LDA.w #$0006 : STA.l $000000, x : INX #2
    LDA.w #$0080 : STA.l $000000, x : INX #2
    LDA.w #$2100 : STA.l $000000, x : INX #2

    ; Set BG3 to start at $4000 (8000 in vram)
    LDA.w #$0006 : STA.l $000000, x : INX #2
    LDA.w #$0004 : STA.l $000000, x : INX #2
    LDA.w #$210C : STA.l $000000, x : INX #2

    ; Set BG3 to start at $4000 (8000 in vram)
    LDA.w #$0006 : STA.l $000000, x : INX #2
    LDA.w #$0004 : STA.l $000000, x : INX #2
    LDA.w #$212C : STA.l $000000, x : INX #2

    ; VRAM DMA <source addr>, <source bank>, <dest addr>, <size>
    LDA.w #$0001 : STA.l $000000, x : INX #2
    LDA.w #cm_hud_table : STA.l $000000, x : INX #2
    LDA.w #cm_hud_table>>16 : STA.l $000000, x : INX #2
    LDA.w #$4000 : STA.l $000000, x : INX #2
    LDA.w #$0900 : STA.l $000000, x : INX #2

    ; Disable force-blank
    LDA.w #$0006 : STA.l $000000, x : INX #2
    LDA.w #$000f : STA.l $000000, x : INX #2
    LDA.w #$2100 : STA.l $000000, x : INX #2
    ...
    ; Wait for SNES to process these commands
-
    LDA.l SNES_CMD_PTR
...
-
    ...
    ; Set menu palette
    LDA #$7277 :  STA.l $00328A                ; light pink
    LDA #$0000 :  STA.l $00328E : STA.l $003296  ; black
    STA.l $00329A : STA.l $0032B6 : STA.l $0032BA
    LDA #$48F3 :  STA.l $003292                ; dark pink
    LDA #$7FFF :  STA.l $003294 : STA.l $00329C  ; white
    LDA #$4376 :  STA.l $0032B2                ; light green
    LDA #$761F :  STA.l $0032B4 : STA.l $0032BC  ; pink
    ...
    ; CGRAM DMA
    LDA.w #$0007 : STA.l $000000, x : INX #2
    LDA.w #$3200 : STA.l $000000, x : INX #2
    LDA.w #$0000 : STA.l $000000, x : INX #2
    LDA.w #$0200 : STA.l $000000, x : INX #2
    LDA.w #$0000 : STA.l $000000, x
...
cm_tilemap_menu:
{
  ...
  .continue
    ...
    LDA [!DP_CurrentMenu] : AND #$7FFF : TAX
...
cm_tilemap_transfer:
{
    ...
    LDA.w #$0001 : STA.l $000000, x : INX #2
    LDA.w #!ram_tilemap_buffer : STA.l $000000, x : INX #2
    LDA.w #!ram_tilemap_buffer>>16 : STA.l $000000, x : INX #2
    LDA.w #$0000 : STA.l $000000, x : INX #2
    LDA.w #$0800 : STA.l $000000, x : INX #2
    LDA.w #$0000 : STA.l $000000, x
...
cm_ctrl_mode:
{
    ...
    LDA.l !IH_CONTROLLER_PRI

    ; set palette
    %a8() : LDA #$28 : STA !DP_Palette : %a16()

    LDA.l !IH_CONTROLLER_PRI : BEQ .clear_and_draw
    ...
    LDA.l !IH_CONTROLLER_PRI : STA [!DP_CtrlInput]
    ;JSL GameModeExtras
    %sfxconfirm()
    BRA .exit
...
cm_go_back:
{
    ...
    LDA.l !ram_cm_stack_index : DEC #2 : BPL .done
    ...
    LDA #$0000
  .done
    STA.l !ram_cm_stack_index    
    LDA.l !ram_cm_stack_index : BNE .end
...
+   LDA.l !IH_CONTROLLER_PRI_NEW : BEQ .check_holding

    ; Initial delay of $0E frames
    LDA #$000E : STA !ram_cm_input_timer

    ; Return the new input
    LDA.l !IH_CONTROLLER_PRI_NEW
    RTS

  .check_holding
    ; Check if we're holding the dpad
    LDA.l !IH_CONTROLLER_PRI : AND #$0F00 : BEQ .noinput
    ...
    LDA.l !IH_CONTROLLER_PRI : AND #$0F00 : ORA !IH_INPUT_HELD
    RTS
...
cm_move:
{
  ...
  .inBounds
    ...
    ; check for blank menu line
    LDA [!DP_MenuIndices],Y
    CMP #$FFFF : BEQ .repeat

    ; Load the action from the menu item
    TAX
    LDA $0000,X
    CMP #$8000 : BCS .repeat
    BRA .end
...
cm_execute:
{
    ...
    ; Check if the draw-only flag is set, in that case we don't execute the action
    CMP #$8000 : BCS .end
...
execute_toggle:
{
    ...
    ; If JSL target = $FFFF, then skip all interactions
    LDA !DP_JSLTarget : CMP #$FFFF : BEQ .end
...
cm_hex2dec:
{
    ...
    LDA $2306 : STA !DP_SecondDigit
    LDA $2308 : STA !DP_ThirdDigit
    LDA !DP_Temp : STA $2251
